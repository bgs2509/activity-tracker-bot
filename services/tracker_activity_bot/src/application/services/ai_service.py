...
⚠️ КРИТИЧЕСКИ ВАЖНО - ВАЛИДАЦИЯ ВРЕМЕНИ ОТНОСИТЕЛЬНО ТЕКУЩЕГО МОМЕНТА:
- ЕСЛИ end_time получается В БУДУЩЕМ (позже текущего времени {current_time} по часовому поясу {user_timezone}):
  → АВТОМАТИЧЕСКИ замени end_time на ТЕКУЩЕЕ ВРЕМЯ (в UTC формате)
  → Пользователь скорее всего имел в виду "до сейчас", но указал будущее время по ошибке
  → Это НЕ ошибка парсинга - просто исправь тихо и сохрани confidence "high"
  → Пример: если сейчас 10:17, а пользователь написал "до 11:17" - замени на текущее время
- start_time тоже не может быть в будущем - если получается будущее, используй логику из правил ниже
...
3. ЛОГИКА ОПРЕДЕЛЕНИЯ ВРЕМЕНИ КОГДА НЕ УКАЗАНО:
   ...
   - Если последняя активность была давно ИЛИ нет данных:
     ...
     → start_time = текущее время - разумная продолжительность
     → end_time = текущее время
   - Если указана только продолжительность без времени:
     → end_time = текущее время
     → start_time = текущее время - указанная продолжительность
...
Альтернативы тоже должны иметь валидное время (end_time > start_time).
...
ВАЖНО: Все поля времени (start_time, end_time) ОБЯЗАТЕЛЬНЫ и должны быть валидными ISO 8601 строками в UTC.
