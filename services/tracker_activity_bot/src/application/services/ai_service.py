...
class AIService:
    ...
    async def parse_activity_text(
        ...
        while current_model and attempts < max_retries:
            ...
            try:
                ...
                # Log prompt details (DEBUG level - can be very long)
                logger.debug(
                    "AI prompt prepared",
                    extra={
    ...
    async def _call_ai_with_timeout(
        ...
        try:
            ...
            # Log raw AI response (DEBUG level)
            ...
            # Log parsed result details (INFO level)
            logger.info(
                "AI response received and parsed",
                extra={
                    "model": model,
                    "response_length": len(content),
                    "category_selected": data.get("category"),
                    "confidence": data.get("confidence"),
                    "has_alternatives": len(data.get("alternatives", [])) > 0
                }
            )
    ...
    def _build_prompt(
        ...
        # Format categories (without emojis - they're stored separately in DB)
        categories_text = "\n".join([
            f"- {cat['name']}"
...
ПРИМЕРЫ ПРАВИЛЬНОГО СОПОСТАВЛЕНИЯ (few-shot learning):

Примеры для категории "Работа":
- "созвон с командой" → Работа
- "писал отчёт" → Работа
- "встреча с клиентом" → Работа
- "код-ревью коллеги" → Работа
- "planning meeting" → Работа

Примеры для категории "Обучение":
- "смотрел курс по Python" → Обучение
- "читал книгу про архитектуру" → Обучение
- "проходил туториал по React" → Обучение
- "изучал документацию API" → Обучение

Примеры для категории "Хобби-кодинг":
- "писал pet-проект" → Хобби-кодинг
- "изучал новый фреймворк для себя" → Хобби-кодинг
- "решал задачки на leetcode" → Хобби-кодинг
- "эксперименты с новой библиотекой" → Хобби-кодинг

Примеры для категории "Отдых":
- "смотрел сериал" → Отдых
- "играл в игры" → Отдых
- "читал художественную книгу" → Отдых
- "слушал музыку" → Отдых
- "гулял в парке" → Отдых

Примеры для категории "Спорт":
- "бегал в парке" → Спорт
- "тренировка в зале" → Спорт
- "йога" → Спорт
- "плавание" → Спорт
- "футбол с друзьями" → Спорт

Примеры для категории "Транспорт":
- "ехал на работу" → Транспорт
- "дорога домой" → Транспорт
- "поездка в метро" → Транспорт
- "в пробке простоял" → Транспорт

Примеры для категории "Еда":
- "завтракал" → Еда
- "обед" → Еда
- "готовил ужин" → Еда
- "перекус" → Еда

Примеры для категории "Семья":
- "играл с детьми" → Семья
- "разговор с родителями" → Семья
- "семейный ужин" → Семья
- "помогал родственникам" → Семья

Примеры для категории "Сон":
- "спал" → Сон
- "дремал" → Сон
- "дневной сон" → Сон

⚠️ ВАЖНО: Эти примеры для демонстрации логики выбора категорий.
